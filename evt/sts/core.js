...
export const attackCore = {
  ...
  triggers: [{ event: 'entity:attack', order: 0, script: `
State.emit('entity:damage', { target = Event.target, amount = Event.amount, source = Event.source })
` }],
};

export const lossCore = {
  id: 'core:entity:loss',
  triggers: [{ event: 'entity:loss', order: 0, script: `
...
if Event.target ~= 'player' then
  State.emit('enemy:loss', { target = Event.target, actualLoss = loss })
end
` }],
};
...
export const damageCore = {
  ...
  triggers: [{ event: 'entity:damage', order: 0, script: `
...
export const damageLossCore = {
  ...
  triggers: [{ event: 'entity:damage', order: -9999, script: `
local net = Event.actualDamage or 0
if net > 0 then
  State.emit('entity:loss', { target = Event.target, amount = net, source = Event.source })
end
...
export const entityDieEmitterCore = {
  ...
  triggers: [{ event: 'entity:loss', order: -9999, script: `
...
export const healCore = {
  id: 'core:entity:heal',
  triggers: [{ event: 'entity:heal', order: 0, script: `
local cur   = State.get('entities', Event.target, 'hp')    or 0
...
export const blockCore = {
  ...
  triggers: [{ event: 'entity:block', order: 0, script: `
...
export const entityDieCore = {
  id: 'core:entity:die',
  triggers: [{ event: 'entity:die', order: 0, script: `
if Event.target == 'player' then
...
// 敌人受伤未死时触发 onLoss AI（HP 阈值检测）
export const enemyLossAiCore = {
  id: 'core:enemy:loss:ai',
  triggers: [{ event: 'enemy:loss', order: 0, script: `
local hp = State.get('entities', Event.target, 'hp') or 0
if hp > 0 then
  State.emit('enemy:ai', { target = Event.target, phase = 'onLoss' })
end
` }],
};
...
export const battleStartCore = {
  ...
  triggers: [{ event: 'battle:start', order: 0, script: `
-- 绑定所有敌人并进行初始化 AI
for slot = 1, 10 do
  local eid = State.get('enemies', tostring(slot))
  if eid ~= nil then
    ...
    if typeId then
      ...
      State.emit('enemy:ai', { target = eid, phase = 'init' })
    end
  end
end
...
export const battleEndCore = {
  id: 'core:battle:end',
  triggers: [{ event: 'battle:end', order: 0, script: `
...
export const cardMoveCore = {
  id: 'core:card:move',
  triggers: [{ event: 'card:move', order: 0, script: `
...
export const cardDrawCore = {
  id: 'core:card:draw',
  triggers: [{ event: 'card:draw', order: 0, script: `
...
export const cardDiscardCore = {
  ...
  triggers: [{ event: 'card:discard', order: 0, script: `
State.emit('card:move', {
  from       = 'hand',
  to         = 'discardPile',
...
export const cardExhaustCore = {
  ...
  triggers: [{ event: 'card:exhaust', order: 0, script: `
State.emit('card:move', {
...
export const statusApplyCore = {
  ...
  triggers: [{ event: 'status:apply', order: 0, script: `
...
export const statusRemoveCore = {
  ...
  triggers: [{ event: 'status:remove', order: 0, script: `
...
export const playerTurnStartCore = {
  ...
  triggers: [{ event: 'player:turn:start', order: 0, script: `
...
export const turnCounterCore = {
  ...
  triggers: [{ event: 'player:turn:start', order: 1000, script: `
...
export const playerTurnEndCore = {
  ...
  triggers: [{ event: 'player:turn:end', order: 0, script: `
...
export const cardPlayCore = {
  id: 'core:card:play',
  triggers: [{ event: 'card:play', order: 0, script: `
...
export const cardPlayCleanupCore = {
  ...
  triggers: [{ event: 'card:play', order: -400, script: `
...
export const turnSequenceCore = {
  ...
  triggers: [{ event: 'turn:end', order: 0, script: `
...
for _, slot in ipairs(enemySlots()) do
  ...
  if eid ~= nil then
    ...
    if getEnemyId(slot) ~= nil then
      State.emit('enemy:ai', { target = eid, phase = 'update' })
...
export const reshuffleCore = {
  ...
  triggers: [{ event: 'deck:deplete', order: 0, script: `
...
export const cardCreateCore = {
  id: 'core:card:create',
  triggers: [{ event: 'card:create', order: 0, script: `
