 ...
 *   hooks      — 统一 hook 表；卡牌主要使用 'event:<eventName>'
 */
...
`,
    // entity:loss：在 lossCore(0) 扣完 HP 后、dieEmitter(-9999) 前触发
    // actualLoss = min(净伤, 剩余HP)，击杀时不会多回血
    // 只处理敌人实际失血，排除玩家因荆棘/主动失血造成的 loss
    'event:entity:loss': `
if Event.target == 'player' then return end
