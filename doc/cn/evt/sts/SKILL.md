...
状态通过 `Ctx.self` 表达实例身份。身份过滤在 hook 定义里用 `match` 字段声明（如 `match: { target: 'self' }`），而不是在脚本开头写 guard。
...
`actions` 主要是意图展示数据；真正行为仍写在 `hooks` 里。
...
### 6. `actions{}` 和 `hooks` 是两层

敌人的 `actions{}` 主要描述 intent 展示；
真正行动逻辑写在 `event:enemy:action` 与 `event:enemy:update` hooks 里；
受伤阈值、被打反应这类逻辑通常直接挂在 `event:entity:loss`。
...
- 卡牌和状态的常见事件挂点：`event:card:effect`、`event:actor:turn:start/end`
- 敌人通常使用 `event:enemy:action`、`event:enemy:update`，受伤反应常挂 `event:entity:loss`
